...
def fetch_all(settings, smoke: bool) -> list[NewsItem]:
    ...
    rss_urls = {k: v for k, v in rss_cfg.items() if isinstance(v, list)}
    items.extend(rss.fetch(rss_urls, lookback_hours=rss_cfg.get("lookback_hours", 48)))
...
def _publish_one(item: NewsItem, score: float, settings, rules, llm, rewriter_prompt, cta, store,
    ...
    booking_url = cta.get("booking_url", "https://cal.com/suki-systems/30min")
